...
namespace App\Http\Modulos\RepresentacionesGraficas\Documentos\etl_pencundi\rg900594384;
...
class Rg900594384_1 extends RgBase
{
    ...
    public function getPdf()
    {
        ...
        $datosComprobante['nota_cabecera_1'] = "";
        if(isset($ofe_representacion_grafica->nota_cabecera_1) && $ofe_representacion_grafica->nota_cabecera_1 != ""){
            $datosComprobante['nota_cabecera_1'] = $ofe_representacion_grafica->nota_cabecera_1;
        }

        $datosComprobante['nota_cabecera_2'] = "";
        if(isset($ofe_representacion_grafica->nota_cabecera_2) && $ofe_representacion_grafica->nota_cabecera_2 != ""){
            $datosComprobante['nota_cabecera_2'] = $ofe_representacion_grafica->nota_cabecera_2;
        }
        ...
        if ($cdo_tipo == "FC") {
            ...
            // informacion ofe
            $fpdf->setXY($posx + 55,$posy+2);
            $fpdf->SetFont('Arial','B',8);
            $fpdf->MultiCell(75,3.7,utf8_decode($oferente),0,'C');
            $fpdf->setX($posx + 43);
            $fpdf->MultiCell(100,3.5,"NIT. ".$datosComprobante['ofe_nit'],0,'C');
            $fpdf->setX($posx + 43);
            $fpdf->MultiCell(100,3.5,utf8_decode($datosComprobante['nota_cabecera_1']),0,'C');
            $fpdf->setX($posx + 43);
            $fpdf->MultiCell(100,3.5,utf8_decode($datosComprobante['nota_cabecera_2']),0,'C');
            $fpdf->setX($posx + 43);
            $fpdf->MultiCell(100,3.5,utf8_decode("CORREO: ".$ofe_correo),0,'C');
            $fpdf->setX($posx + 43);
            $fpdf->MultiCell(100,3.5,utf8_decode("DIRRECCIÓN: ".$ofe_dir),0,'C');
            $fpdf->setX($posx + 43);
            $fpdf->MultiCell(100,3.5,utf8_decode("TEL: ".$ofe_tel),0,'C');

            //Logos
            $fpdf->Image($this->getFullImage('logo'.$ofe_identificacion.'.png'), $posx - 5, $posy+1, 42);

            //Frase Logo
            // $fpdf->Image($this->getFullImage('logo_frase.png'), $posx + 135, $posy-1, 45);
            
            // resolucion
            $fpdf->setXY($posx+140, $posy+27.5);
            $fpdf->SetFont('Arial','B',8);
            ...
            $fpdf->setX($posx+1);
            $fpdf->SetFont('Arial','B',8);
            $fpdf->Cell(32,4,utf8_decode("Correo Electrónico:"),0,0,'L');
            $fpdf->SetFont('Arial','',8);
            $fpdf->MultiCell(70,4,utf8_decode($adq_correo),0,'L');
            $fpdf->Ln(1);
            $fpdf->setX($posx+1);
            $fpdf->SetFont('Arial','B',8);
            ...
            $posy = $fpdf->getY()+1;
            $fpdf->setXY($posx, $posy+1);
            $fpdf->MultiCell(181,3.5,"Observaciones: ".utf8_decode(implode("\n",$datosComprobante['observacion'])),0,'L');
            $fpdf->Rect($posx, $posy, 181, 18.5);
            ...
            $posy = $fpdf->getY();
            $fpdf->setXY($posx, $posy+1);
            $fpdf->MultiCell(195,3.5,"Observaciones: ".utf8_decode(implode("\n",$datosComprobante['observacion'])),0,'L');
            $fpdf->Rect($posx, $posy, 195, 20);
            $fpdf->SetFont('Arial', '', 7);
