...
class PDF900594384_1 extends PDFBase {
    ...
    function Header() {
        ...
        if ($this->datosComprobante['cdo_tipo'] != "FC") {
            ...
            //logo
            $this->Image($this->imageHeader, $posx + 2, $posy + 8, 35);
            $this->Rect($posx, $posy, 40, 35);
            
            //Informacion OFE
            $this->setXY($posx + 42, $posy);
            $this->SetFont('Arial', 'B', 10);
            $this->MultiCell(110, 5, utf8_decode($this->datosComprobante['oferente']), 0, 'C');
            $this->Rect($posx + 40, $posy, 115, 35);
            $this->SetFont('Arial', 'B', 9);
            $this->setXY($posx + 40, $posy + 12);
            $this->MultiCell(110, 3.5, utf8_decode("NIT: " . $this->datosComprobante['ofe_nit']), 0, 'C');
            $this->ln(1);
            $this->setX($posx + 40);
            $this->MultiCell(110, 3.5, utf8_decode($this->datosComprobante['nota_cabecera_1']), 0, 'C');
            $this->ln(1);
            $this->setX($posx + 40);
            $this->MultiCell(110, 3.5, utf8_decode($this->datosComprobante['nota_cabecera_2']), 0, 'C');
            $this->ln(1);
            $this->setX($posx + 40);
            $this->MultiCell(110, 3.5, utf8_decode("CORREO: " . $this->datosComprobante['ofe_correo']), 0, 'C');
            $this->ln(1);
            $this->setX($posx + 40);
            $this->MultiCell(110, 3.5, utf8_decode("TEL: " . $this->datosComprobante['ofe_tel']), 0, 'C');

            $this->setXY($posx + 155, $posy);
            $this->MultiCell(40, 15, utf8_decode("Código:"), 1, 'L');
            $this->setX($posx + 155);
            $this->MultiCell(40, 10, utf8_decode("Versión:"), 1, 'L');
            $this->setX($posx + 155);
            $this->MultiCell(40, 10, utf8_decode("Página: " . $this->PageNo() . ' de {nb}'), 1, 'L');
            ...
            $this->setXY($posx, $posy);
            $this->SetFont('Arial', '', 9);
            $this->Cell(40, 5, utf8_decode("CORREO ELECTRÓNICO:"), 0, 0, 'L');
            $this->SetFont('Arial', 'B', 9);
            $this->MultiCell(160, 4, utf8_decode($this->datosComprobante['adq_correo']), 0, 'L');
            $this->Rect($posx,$posy,195,4.5);
            $posy = $this->GetY()+0.5;
            $this->setXY($posx, $posy);
            $this->SetFont('Arial', '', 9);
