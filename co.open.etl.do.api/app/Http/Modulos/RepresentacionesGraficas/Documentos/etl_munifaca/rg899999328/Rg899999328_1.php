...
class Rg899999328_1 extends RgBase
{
    ...
    public function getPdf()
    {
        ...
        // $datosComprobante['logo_footer']  = $this->getFullImage("logo_footer.png");
        ...
        $strEstrato = "";
        if(isset($cdo_informacion_adicional->estrato) && $cdo_informacion_adicional->estrato != ""){
            $strEstrato = $cdo_informacion_adicional->estrato;
        }

        $strNumeroMedidor = "";
        if(isset($cdo_informacion_adicional->numero_medidor) && $cdo_informacion_adicional->numero_medidor != ""){
            $strNumeroMedidor = $cdo_informacion_adicional->numero_medidor;
        }

        $strCuotaInicial = "";
        if(isset($cdo_informacion_adicional->cuota_inicial) && $cdo_informacion_adicional->cuota_inicial != ""){
            $strCuotaInicial = $cdo_informacion_adicional->cuota_inicial;
        }
        ...
        $strPiePagina = "";
        if(isset($cdo_informacion_adicional->pie_pagina) && $cdo_informacion_adicional->pie_pagina != ""){
            $strPiePagina = $cdo_informacion_adicional->pie_pagina;
        }
        ...
        // $items = array_merge($items,$items,$items,$items,$items,$items,$items,$items,$items,$items,$items);
        ...
        if ($cdo_tipo == "FC") {
            ...
            $fpdf->TextWithDirection(11,185,utf8_decode("NOMBRE DEL FABRICANTE DEL SOFTWARE (PROVEEDOR TECNOLÓGICO): ".$razon_social_pt." NIT: ".$nit_pt." NOMBRE DEL SOFTWARE: ".$datosComprobante['nombre_software']),'U');

            $fpdf->SetFont('Arial', 'B', 10);
            $fpdf->TextWithDirection(203,100,utf8_decode("USUARIO"),'D');
            
            //Logos
            // $fpdf->Image($datosComprobante['logo_footer'], $posx, $posy+2, 60);

            // informacion ofe
            $fpdf->setXY($posx + 62,$posy+2);
            $fpdf->SetFont('Arial','B',8);
            $fpdf->MultiCell(60,3.7,utf8_decode($oferente),0,'C');
            $fpdf->Ln(1);
            $fpdf->setX($posx + 62);
            $fpdf->MultiCell(60,3.5,"Nit. ".$datosComprobante['ofe_nit'],0,'C');
            $fpdf->setX($posx + 62);
            $fpdf->MultiCell(60,3.5,utf8_decode($ofe_dir),0,'C');
            $fpdf->setX($posx + 62);
            $fpdf->MultiCell(60,3.5,utf8_decode("PBX: ".$ofe_tel),0,'C');

            //Logos
            $fpdf->Image($this->getFullImage('logo'.$ofe_identificacion.'.png'), $posx+124, $posy+2, 60);

            // resolucion
            $fpdf->setXY($posx+140, $posy+22);
            $fpdf->SetFont('Arial','B',8);
            $fpdf->MultiCell(40,3.5,"FACTURA ELECTRONICA",0,'C');
            $fpdf->Ln(0.5);
            $fpdf->setX($posx+140);
            $fpdf->MultiCell(40,3.5,$cdo_consecutivo,1,'C');
            $fpdf->Ln(1);
            $fpdf->setX($posx);
            ...
            // totales
            $intTotal = ($this->parserNumberController($subtotal) + $this->parserNumberController($iva)) - ($intTotalReteFte + $intTotalReteIva + $intTotalReteIca);
            ...
            $posy += 9;
            $fpdf->setXY($posx, $posy);
            $fpdf->MultiCell(181,4,utf8_decode($strPiePagina),0,'C');
            $fpdf->Ln(2);
            $fpdf->SetFont('Arial', 'B', 7);
            $fpdf->setX($posx);
            ...
            if($signaturevalue != "" && $qr != ""){
                ...
                if ($pic!==false) $fpdf->Image($pic[0], $posx + 145, $posy + 5,33,33, $pic[1]);
                $fpdf->setX($posx);
                ...
                $fpdf->setX($posx);
                $fpdf->SetFont('Arial','B',7);
                $fpdf->Cell(100,3, ($cdo_tipo == "FC") ? "CUFE:" : "CUDE:",0,0,'L');
                $fpdf->Ln(4);
                $fpdf->setX($posx);
                $fpdf->SetFont('Arial','',7);
            ...
            // Banco
            // logo
            $fpdf->Image($this->getFullImage('logo'.$ofe_identificacion.'.png'), $posx, $posy+2, 32);

            $fpdf->SetFont('Arial', 'B', 10);
            $fpdf->TextWithDirection(203,$posy+40,utf8_decode("BANCO"),'D');
            ...
            $fpdf->setX($posx + 27);
            $fpdf->MultiCell(77,3.5,"Dir. ".utf8_decode($ofe_dir)." * Tel(s): ".$ofe_tel,0,'C');
            ...
            $fpdf->SetFont('Arial','B',10);
            $fpdf->MultiCell(37,7,$cdo_consecutivo,0,'C');
            ...
            if($strCodigoBarras != ""){
                ...
                $pic = $fpdf->getImage($dataURI);
                if ($pic !== false) {
                    $fpdf->Image($pic[0],$posx+30,$posy+41,121,20, $pic[1]);
                    $fpdf->setXY($posx, $posy + 63);
                    $fpdf->SetFont('Arial', '', 7);
            ...
            if ($fpdf->GetY() > $posfin) {
                $fpdf->Rect($posx, $fpdf->posy, 195, $posfin - $fpdf->posy + 30);
                $fpdf->Line($posx + 10, $fpdf->posy, $posx + 10, $posfin + 30);
                $fpdf->Line($posx + 26, $fpdf->posy, $posx + 26, $posfin + 30);
                $fpdf->Line($posx + 42, $fpdf->posy, $posx + 42, $posfin + 30);
                $fpdf->Line($posx + 119, $fpdf->posy, $posx + 119, $posfin + 30);
                $fpdf->Line($posx + 135, $fpdf->posy, $posx + 135, $posfin + 30);
                $fpdf->Line($posx + 165, $fpdf->posy, $posx + 165, $posfin + 30);
                $fpdf->AddPage('P', 'Letter');
            ...
            $fpdf->Cell(30, 5, number_format(0, 2, '.', ','), 1, 0, 'R');
            $fpdf->SetTextColor(0);
            $fpdf->ln(5);
            $fpdf->setX($posx + 135);
