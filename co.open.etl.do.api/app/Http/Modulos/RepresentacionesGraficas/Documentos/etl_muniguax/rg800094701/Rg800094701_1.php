...
namespace App\Http\Modulos\RepresentacionesGraficas\Documentos\etl_muniguax\rg800094701;
...
class Rg800094701_1 extends RgBase
{
    ...
    public function getPdf()
    {
        ...
        $telefonoOfe = "";
        if(isset($ofe_representacion_grafica->telefono) && $ofe_representacion_grafica->telefono != ""){
            $telefonoOfe = $ofe_representacion_grafica->telefono;
        }
        ...
        if ($cdo_tipo == "FC") {
            ...
            // informacion ofe
            $fpdf->setXY($posx + 52,$posy-2);
            $fpdf->SetFont('Arial','B',13);
            $fpdf->MultiCell(75,6,utf8_decode(strtoupper($oferente)),0,'C');
            $fpdf->setX($posx + 59);
            $fpdf->MultiCell(60,6,"NIT. ".$datosComprobante['ofe_nit'],0,'C');
            $fpdf->setX($posx + 59);
            $fpdf->MultiCell(60,6,utf8_decode("TEL: ".$telefonoOfe),0,'C');

            //Logos
            $fpdf->Image($this->getFullImage('logo'.$ofe_identificacion.'.png'), $posx+2, $posy-8, 32);
            // $fpdf->Image($this->getFullImage('logo'.$ofe_identificacion.'_1.png'), $posx+142, $posy-12, 38);

            // resolucion
            $fpdf->setXY($posx+140, $posy+24);
            ...
            if($strCodigoBarras != "" && $strReferenciaPago != ""){
                ...
                $fpdf->SetDash(0,0);
                // Logo desprendible
                // $fpdf->Image($this->getFullImage('logo'.$ofe_identificacion.'_1.png'), $posx, $posy+2, 22);
                $fpdf->SetFont('Arial', 'B', 10);
