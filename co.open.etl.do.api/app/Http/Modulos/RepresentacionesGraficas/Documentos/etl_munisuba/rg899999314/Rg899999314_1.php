...
namespace App\Http\Modulos\RepresentacionesGraficas\Documentos\etl_munisuba\rg899999314;
...
class Rg899999314_1 extends RgBase
{
    ...
    public function getPdf()
    {
        ...
        if ($cdo_tipo == "FC") {
            ...
            // informacion ofe
            $fpdf->setXY($posx + 55,$posy+4);
            $fpdf->SetFont('Arial','B',10);
            $fpdf->MultiCell(75,4.5,utf8_decode(strtoupper($oferente)),0,'C');
            $fpdf->setX($posx + 62);
            $fpdf->MultiCell(60,4.5,"NIT. ".$datosComprobante['ofe_nit'],0,'C');
            $fpdf->setX($posx + 62);
            $fpdf->MultiCell(60,4.5,utf8_decode("TEL: ".$ofe_tel),0,'C');

            //Logos
            // $fpdf->Image($this->getFullImage('logo'.$ofe_identificacion.'.png'), $posx - 2, $posy-2, 55);

            // resolucion
            $fpdf->setXY($posx+140, $posy+18);
